...
bool gasDetected = false; // Add this line
...
void initGasSensor() {
    ...
    sensorErrorCount = 0;
    sensorFailure = false;
    gasDetected = false; // Initialize gasDetected
...
bool isGasDetected() {
    ...
    if (sensorFailure) {
        ...
        gasDetected = false; // Ensure gasDetected is false on failure
        return false;
    }
    ...
    if (ratio < THRESHOLD_RATIO) {
        ...
        gasDetected = false; // Reset the flag when no gas is detected
    }
    Serial.println("Gas is not detected, but it is working");
    return false;
}

bool getIsGasDetected() { // Add this getter function
    return gasDetected;
}
...
void resetSensor() {
    ...
    consecutiveAlerts = 0;
    sensorFailure = false;
    gasDetected = false; // Reset gasDetected as well
